...
// ==========================================
// --- YARDIMCI ÇEKİLİŞ FONKSİYONLARI ---
// ==========================================
async function endGiveaway(messageId, contextMessage = null) {
    ...
    if (participantsArray.length === 0) {
        ...
        return channel.send('❌ Çekilişe katılan kimse olmadığından çekiliş iptal edildi.');
    }

    const winners = [];
    const count = Math.min(giveaway.winnerCount, participantsArray.length);
    ...
    const embed = new EmbedBuilder(msg.embeds[0].data)
        .setTitle(`🎉 ÇEKİLİŞ SONA ERDİ: ${giveaway.prize}`)
        .setDescription(`👑 **Kazananlar:** ${winnerMentions}\n🎁 **Ödül:** ${giveaway.prize}`)
        .setColor('#2ECC71');

    await msg.edit({ embeds: [embed], components: [] }).catch(() => {});
    await channel.send(`🎉 Tebrikler ${winnerMentions}! **${giveaway.prize}** ödülünü kazandınız!`);
}

async function rerollGiveaway(messageId, contextMessage) {
    const giveaway = giveaways.get(messageId);
    if (!giveaway) return contextMessage.reply('⚠️ Bu ID ile eşleşen aktif veya geçmiş bir çekiliş bulunamadı.');

    const participantsArray = Array.from(giveaway.participants);
    if (participantsArray.length === 0) return contextMessage.reply('❌ Bu çekilişte başka katılımcı bulunmuyor.');

    const randomWinner = participantsArray[Math.floor(Math.random() * participantsArray.length)];
    const channel = contextMessage.guild.channels.cache.get(giveaway.channelId);

    if (channel) {
        channel.send(`🔄 **Yeniden Çekiliş!** Yeni kazanan: <@${randomWinner}> Tebrikler! 🎉`);
    } else {
        contextMessage.reply(`🔄 Yeni kazanan: <@${randomWinner}>`);
    }
}
...
client.on('messageCreate', async (message) => {
    ...
    // --- BLACKLIST KOMUTU (!blacklist [üye]) ---
    if (message.content.startsWith('!blacklist')) {
        if (!isAdmin) return message.reply('Bu komutu kullanmak için **Yönetici** yetkisine sahip olmalısın!');

        const targetMember = message.mentions.members.first() || message.guild.members.cache.get(message.content.split(' ')[1]);
        if (!targetMember) return message.reply('Lütfen banlanacak geçerli bir kullanıcı etiketleyin veya ID girin!');

        try {
            await targetMember.ban({ reason: `Blacklist komutu ile ${message.author.tag} tarafından yasaklandı.` });
            return message.reply(`🔨 **${targetMember.user.tag}** sunucudan kalıcı olarak banlandı.`);
        } catch (err) {
    ...
    if (message.content.startsWith('!mute')) {
        ...
        else return message.reply('Geçersiz süre formatı!');

        try {
            await targetMember.timeout(msTime, `Mute komutu ile ${message.author.tag} tarafından susturuldu.`);
            return message.reply(`🔇 **${targetMember.user.tag}** adlı kullanıcıya **${timeInput}** süreyle zaman aşımı uygulandı.`);
        } catch (err) {
            console.error('Mute hatası:', err);
            return message.reply('Zaman aşımı uygulanırken hata oluştu.');
        }
    }

    // --- UNMUTE KOMUTU (!unmute [üye]) ---
    if (message.content.startsWith('!unmute')) {
        if (!isAdmin) return message.reply('Bu komutu kullanmak için **Yönetici** yetkisine sahip olmalısın!');

        const targetMember = message.mentions.members.first() || message.guild.members.cache.get(message.content.split(' ')[1]);
        if (!targetMember) return message.reply('Lütfen kullanıcıyı etiketleyin!');

        try {
            await targetMember.timeout(null, `Unmute komutu ile ${message.author.tag} tarafından kaldırıldı.`);
            return message.reply(`🔊 Kullanıcının zaman aşımı kaldırıldı.`);
        } catch (err) {
            console.error('Unmute hatası:', err);
            return message.reply('Zaman aşımı kaldırılırken hata oluştu.');
        }
    }
    ...
    if (content === '!sürüm') return message.reply('📌 **CraftRiva Sunucu Sürümü:** `1.19.3-26.2`');
    ...
    if (content === '!kapat') {
        ...
        if (!isTicketChannel) return message.reply('⚠️ Bu komut sadece destek talebi kanallarında kullanılabilir!').catch(() => {});
    ...
    if (message.content.startsWith('!çekiliş-bitir')) {
        if (!isAdmin) return message.reply('Yönetici yetkisi gerekli!');
        const msgId = message.content.split(' ')[1];
        if (!msgId) return message.reply('Mesaj ID\'si belirtin.');
...
client.on('interactionCreate', async (interaction) => {
        ...
        if (interaction.customId.startsWith('ticket_')) {
            ...
            if (action === 'confirm_close') {
                ...
                try {
                    ...
                    if (ticketOwner && !ticketOwner.user.bot) {
                        try {
                            await ticketOwner.send({
                                content: `Merhaba ${ticketOwner}, destek talebiniz kapatıldı. Görüşme kayıtları ekteki dosyadadır.`,
                                files: [attachment]
                            });
                        } catch (dmErr) {}
                    }
                    ...
                    console.error('Transcript hatası:', err);
                }
            ...
            if (action === 'hile-bildirim') {
                const modal = new ModalBuilder().setCustomId('modal_hile_bildirim').setTitle('Hile Şikayeti');
                modal.addComponents(
                    ...
                    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('kanit_link').setLabel('Kanıt (Link)').setStyle(TextInputStyle.Short).setRequired(true)),
                    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('aciklama').setLabel('Açıklama').setStyle(TextInputStyle.Paragraph).setRequired(true))
                );
                return await interaction.showModal(modal);
            }

            if (action === 'yetkili-sikayet') {
                const modal = new ModalBuilder().setCustomId('modal_yetkili_sikayet').setTitle('Yetkili Şikayeti');
                modal.addComponents(
                    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('yetkili_ismi').setLabel('Yetkili İsmi').setStyle(TextInputStyle.Short).setRequired(true)),
                    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('aciklama').setLabel('Açıklama').setStyle(TextInputStyle.Paragraph).setRequired(true))
                );
                return await interaction.showModal(modal);
            }
            ...
            try {
                ...
                await interaction.reply({ content: '❌ Kanal oluşturulurken hata oluştu!', flags: MessageFlags.Ephemeral });
            }
        }
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.customId === 'modal_hile_bildirim' || interaction.customId === 'modal_yetkili_sikayet') {
            const isHile = interaction.customId === 'modal_hile_bildirim';
            const action = isHile ? 'hile-bildirim' : 'yetkili-sikayet';
            const categoryName = isHile ? 'Hile Bildirimi' : 'Yetkili Şikayeti';
            
            const field1 = isHile ? interaction.fields.getTextInputValue('oyuncu_ismi') : interaction.fields.getTextInputValue('yetkili_ismi');
            const field2 = isHile ? interaction.fields.getTextInputValue('kanit_link') : null;
            const aciklama = interaction.fields.getTextInputValue('aciklama');

            const channelName = `${action}-${interaction.user.username.toLowerCase().replace(/[^a-z0-9]/g, '')}`;

            const existingChannel = interaction.guild.channels.cache.find(c => c.name === channelName);
            if (existingChannel) return interaction.reply({ content: `⚠️ Zaten açık bir talebiniz var: ${existingChannel}`, flags: MessageFlags.Ephemeral });
            ...
            try {
                ...
                console.error('Modal kanal oluşturma hatası:', err);
                await interaction.reply({ content: '❌ Talep oluşturulurken hata meydana geldi!', flags: MessageFlags.Ephemeral });
            }
        }
    }
});
